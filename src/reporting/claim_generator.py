...
def generate_claim_summary(main_table: pd.DataFrame, output_dir: Path | None = None) -> dict[str, object]:
    ...
    if prism_rows.empty:
        raise ValueError("PRISM row not found in main comparison table")
    ...
    baselines_better_than_prism_mae = (
        main_table.loc[main_table["mae_mean"] < float(prism["mae_mean"]), "model"].astype(str).tolist()
    )
    baselines_better_than_prism_rmse = (
        main_table.loc[main_table["rmse_mean"] < float(prism["mae_mean"]), "model"].astype(str).tolist()
    )
    ...
    if prism_best_mae and prism_best_rmse:
        recommended_claim = "PRISM achieves the lowest aggregate MAE and RMSE among all evaluated baselines under the chronological protocol."
        limitations_note = "No limitations on point-forecast accuracy."
    else:
        better_list = ", ".join(baselines_better_than_prism_mae) if baselines_better_than_prism_mae else "none"
        worse_list = ", ".join(baselines_worse_than_prism_mae) if baselines_worse_than_prism_mae else "none"
        recommended_claim = (
            f"PRISM outperforms {worse_list} on MAE, while {better_list} remain strong short-horizon competitors. "
            ...
            "not raw point-forecast dominance."
        )
        limitations_note = (
    ...
    summary_text = "\n".join(
        [
            ...
            f"- Baselines better than PRISM on MAE: {', '.join(baselines_better_than_prism_mae) if baselines_better_than_prism_mae else 'none'}",
            f"- Baselines better than PRISM on RMSE: {', '.join(baselines_better_than_prism_rmse) if baselines_better_than_prism_rmse else 'none'}",
            f"- Baselines worse than PRISM on MAE: {', '.join(baselines_worse_than_prism_mae) if baselines_worse_than_prism_mae else 'none'}",
            "",
            "## Recommended Claim",
            "",
            recommended_claim,
    ...
    payload = {
        ...
        "baselines_better_than_prism_mae": baselines_better_than_prism_mae,
        "baselines_better_than_prism_rmse": baselines_better_than_prism_rmse,
        "baselines_worse_than_prism_mae": baselines_worse_than_prism_mae,
        "recommended_claim": recommended_claim,
        "limitations_note": limitations_note,
    }

    target_root = output_dir or scie_path("outputs", "scie_revision", "reports")
