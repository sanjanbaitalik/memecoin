...
class _TCNModel(nn.Module):
    def __init__(self, input_size: int, hidden_size: int, n_layers: int = 3, kernel_size: int = 3, dropout: float = 0.1):
        super().__init__()
        layers = []
...
def train_sequence_model(
    ...
    X_train, y_train = _to_sequences(train_x, train_y, lookback)
    X_val, y_val = _to_sequences(val_x, val_y, lookback) if val_x.size > 0 else (np.empty((0, lookback, input_size)), np.empty(0))
    X_test, y_test = _to_sequences(test_x, np.zeros(len(test_x)), lookback)
    ...
    fallback_used = bool(np.all(np.isnan(full_pred)))
    if fallback_used:
        full_pred = np.repeat(float(train["price"].iloc[-1]) if len(train) > 0 else 0.0, len(test))
